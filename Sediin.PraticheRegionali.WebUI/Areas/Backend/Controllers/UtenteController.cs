    ...
    public class UtenteController : BaseController
    {
        ...
        public ActionResult UtenteNuovo(string ruolo)
        {
            try
            {
                ...
                _user.AssociaProvincia = _ruolo.Rolename == Roles.Sp_Sindacale.ToString()
        ...
        public async Task<ActionResult> Utente(UtentiViewModel model)
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(model.UserId))
                {
                    ...
                    {
                        if (model.EmailConfermata != null)
                        {
                            _user.EmailConfirmed = (bool)model.EmailConfermata;
                        }
                        else
                        {
        ...
        {
            var _rolesVisibili = ConfigurationProvider.Instance.GetConfiguration().Roles.Where(x => (bool)x.Attivo).Select(v => v.RoleId);
